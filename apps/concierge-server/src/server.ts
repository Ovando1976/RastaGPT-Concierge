...
function tool_find_recipe(args: { query?: string }) {
  ...
  if (!q) return RECIPES;
  return RECIPES.filter(r => r.name.toLowerCase().includes(q) || r.ingredients.some(i => i.toLowerCase().includes(q)));
}
...
function tool_request_ride(args: { request: { to: string; pax?: number; island?: string; when?: string }}) {
  ...
  return { ok:true, to:r.to, pax:r.pax??1, island:r.island??"St. Thomas", when:r.when??"ASAP",
           fare_estimate_usd:fare, confirmation:code, driver:{ name:"Island Driver", phone:"+1-340-555-1212" } };
}
...
app.use(cors({
  origin: [
    "http://localhost:3000",
    "https://rasta-gpt-concierge.vercel.app"
  ],
  methods: ["GET","POST"],
}));
app.use(express.json());

// logging + basic rate limiting
app.use(morgan("dev"));
...
function invokeHandler(req: Request, res: Response) {
  ...
  try {
    switch (name) {
      ...
      default: return res.status(400).json({ ok:false, error:"unknown_tool", name });
    }
  } catch (e:any) {
