...
export function estimateReservationUsd(
  ...
  // Reserve a small safety margin so a request cannot silently consume an
  // arbitrarily large amount. maxOutputTokens is the primary hard bound.
...
export async function reserveUsage(input: {
  ...
  const [userMeter, platformMeter] = await Promise.all([
    getDocument(userMeterPath),
    getDocument(platformMeterPath),
  ]);
